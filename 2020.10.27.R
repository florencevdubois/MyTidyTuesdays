...
# mildenberger's data
mildenberger <- read.csv("mildenberger-data.csv", sep = ";") %>% # get the opinion data
  filter(!GEOID == "National") %>% # remove Canada obs
  mutate(GEOID = as.numeric(levels(GEOID))[GEOID]) %>% 
  filter(!GEOID < 100) %>% # remove provinces
  filter(GEOID >= 11000 & GEOID <= 13999) # keep NS, NB and PEI only

# Canada shapefiles
boundaries <- readOGR(dsn = "/your-path",
                      layer = "your-boundaries-shapefile-without-extension") # I did a custom export from this website: http://geo1.scholarsportal.info/#r/details/_lang=fr&_uri@=2907074758 (you can select a zone by drawing on the map and export only this location)
...
ggplot(ridings_opinion, aes(x = long, y = lat)) +
  ...
  labs(title = "Location of Wind Turbines in New Brunswick, Nova Scotia and Prince Edward Island",
       ...
       caption = "Wind turbines data: Government of Canada\nOpinion data: Mildenberger et al. https://www.umontreal.ca/climat/index.html\nData viz: @florencevdubois") +
